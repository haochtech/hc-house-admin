...
if($navlist)
		...
		$condition = ' WHERE `uniacid` = :uniacid  AND isrecommand = 1 AND status =0   AND `cityid` = :cityid  AND (ispub = 1 AND ischeck =1 OR ispub = 0) ORDER BY sort DESC LIMIT  '.$intro['pailimit'];
		$params = array(':uniacid' => $_W['uniacid'] ,':cityid'=>$cityinfo['id']);
 ...
 //echo "<pre>";	print_r($houselist);die;
return $this->result(0, 'success', array('banner'=>$banner, 'intro'=>$intro,'newhouselist'=>$newhouselist,'oldhouselist'=>$oldhouselist,'agentlist'=>$agentlist,'lethouselist'=>$lethouselist,'navlist'=>$navlist,'cityinfo'=>$cityinfo,'houselist'=>$houselist));
...
$houseinfo = pdo_get('weixinmao_house_lethouseinfo',array('id'=>$v['houseid']));
  	...
  	public function doPageMycomment()
	{
		...
		$uid = $_GPC['uid'];
		$sessionid = $_GPC['sessionid'];
		$condition = ' WHERE c.uniacid = '.$_W['uniacid']. ' AND c.uid='.$uid ;

		$sqllist = "SELECT c.*,h.housename as title,date(FROM_UNIXTIME(c.createtime)) as createtime ";
		
		$sql = " FROM " . tablename('weixinmao_house_comment') . " AS c ";
		
		$sql .= "  LEFT JOIN  " . tablename('weixinmao_house_houseinfo') . " AS h ON h.id = c.pid ";
		//var_dump($sqllist.$sql.$condition);exit;
		$list = pdo_fetchall($sqllist.$sql.$condition);
        $intro = pdo_fetch("SELECT maincolor FROM " . tablename('weixinmao_house_intro')." WHERE   uniacid=:uniacid",array(":uniacid" => $_W['uniacid']));
        $data = array('list'=>$list ,'intro'=>$intro);
		return $this->result(0, 'success', $data);
